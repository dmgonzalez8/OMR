...
def get_model_densenet(num_classes):
    ...
    # DenseNet201 has slightly different block configurations
    return_layers = {
        'norm5': '0'  # Assuming 'norm5' is the last normalization layer in DenseNet201
    }
    
    # Create the FPN on top of the DenseNet backbone
    in_channels_stage2 = backbone.norm5.num_features  # 'norm5' layer number of features
    out_channels = 256  # Commonly used size for FPN outputs
    backbone_with_fpn = FPN(backbone, return_layers, in_channels_stage2, out_channels)
    
    # Define the anchor generator with appropriate sizes and aspect ratios
    anchor_generator = AnchorGenerator(sizes=((32, 64, 128, 256, 512),),
                                       aspect_ratios=((0.5, 1.0, 2.0),))
    
    # Create the ROI heads with a FastRCNNPredictor
    roi_pooler = torchvision.ops.MultiScaleRoIAlign(featmap_names=['0'],
                                                    output_size=7,
                                                    sampling_ratio=2)
    box_predictor = FastRCNNPredictor(out_channels * 4, num_classes)  # Adjust the number of inputs

    # Create the Faster R-CNN model
    model = FasterRCNN(backbone_with_fpn,
                       num_classes=None,  # We handle num_classes with our box_predictor
